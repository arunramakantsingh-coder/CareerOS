...
def _template_to_schema(value: Any) -> dict[str, Any]:
    """Turn the editable JSON example/template into a provider JSON Schema.

    The lab accepts either a normal JSON example or an explicit JSON Schema. This keeps the
    visible contract human-friendly while still exercising the same structured-output path used
    by the production CV reconciliation workflow.
    """
...
def _profile_for(user: User, db: Session) -> CandidateProfile:
    profile = (
        ...
        .filter(CandidateProfile.user_id == user.id, CandidateProfile.is_active.is_(True))
        .first()
    )
    if not profile:
...
def _cv_documents(profile: CandidateProfile, db: Session) -> list[Document]:
    return (
        db.query(Document)
        .filter(Document.candidate_id == profile.id, Document.document_category == "cv")
        .order_by(Document.created_at.desc())
        .all()
    )


def _source_payload(document: Document) -> dict[str, Any]:
    text = _prepare_cv_text((document.source_metadata or {}).get("extracted_text", ""))
    return {
        "id": str(document.id),
...
def source_documents(user: User = Depends(require_developer), db: Session = Depends(get_db)):
    ...
    return {
        "selected_document_id": str(documents[0].id) if documents else None,
        "documents": [_source_payload(document) for document in documents],
    }


@router.post("/extract")
async def extract_cv_json(
    request: CVJsonLabRequest,
    ...
    # The prompt itself is compact JSON. The provider also receives response_schema so the
    # editable JSON contract is the actual structured-output contract, not merely a prompt hint.
    prompt = _compact(
        {
    ...
    # Exercise the same canonical normalization and profile-mapping path used by reconciliation,
    # but inside a savepoint that is always rolled back. This proves whether the returned JSON can
    # be understood by CareerOS without mutating the real Professional Profile.
    ...
    return {
        "status": "completed",
        "document": _source_payload(document),
        ...
        "application_mapping": {
            ...
            "normalized_sections": list(normalized.keys()),
        },
        "metrics": {
            "cv_chars": len(text),
            "instruction_chars": len(compact_instruction),
            "template_chars": len(request.template.__str__()),
            "compact_template_chars": len(compact_template),
