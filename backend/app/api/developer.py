...
def reset_profile(user: User = Depends(require_developer), db: Session = Depends(get_db)):
    ...
    db.add(AuditLog(user_id=user.id, tenant_id=user.tenant_id, action="DEVELOPER_RESET_PROFILE", entity_type="candidate_profile", entity_id=profile.id, details={"documents_removed": len(documents), "login_preserved": True}))
    db.commit()
    return {"reset": True, "documents_removed": len(documents), "personas_removed": "all", "email_connectors_reset": True, "login_preserved": True}
