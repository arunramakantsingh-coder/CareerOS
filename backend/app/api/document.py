...
from app.utils.document_processing import STORAGE_ROOT, extract_zip, save_document, sha256_bytes, is_supported
...
def get_or_create_profile(current_user: User, db: Session):
    ...
    if not profile:
        profile = CandidateProfile(user_id=current_user.id, full_name=current_user.name, primary_email=current_user.email, reconciliation_status='pending'); db.add(profile); db.commit(); db.refresh(profile)
    return profile
...
def create_record(profile, name: str, content: bytes, db: Session, category: Optional[str] = None, subcategory: Optional[str] = None, source: str = 'upload', source_metadata: Optional[dict] = None, extract: bool = True):
    digest = sha256_bytes(content); duplicate_of = None
    for item in db.query(Document).filter(Document.candidate_id == profile.id).all():
        if (item.source_metadata or {}).get('sha256') == digest: duplicate_of = str(item.id); break
    stored_path, _ = save_document(str(profile.id), name, content)
    inferred_category, inferred_subcategory = classify(name)
    document = Document(candidate_id=profile.id, filename=Path(stored_path).name, original_filename=name, file_size=len(content), file_type=Path(name).suffix.lower().lstrip('.') or None, storage_path=stored_path, storage_url=f'/api/v1/documents/download/{profile.id}/{Path(stored_path).name}', document_category=category or inferred_category, document_subcategory=subcategory or inferred_subcategory, document_type=inferred_subcategory, status='processing', processing_status={'stage': 'uploaded', 'duplicate_of': duplicate_of}, extraction_status='pending', source=source, source_metadata={**(source_metadata or {}), 'sha256': digest, 'original_path': name})
    db.add(document); db.commit(); db.refresh(document)
    if not extract:
        document.status = 'processed'; document.extraction_status = 'complete'; document.processing_status = {'stage': 'archive-preserved'}; db.commit(); db.refresh(document); return document
    if duplicate_of:
        document.status = 'processed'; document.extraction_status = 'complete'; document.processing_status = {'stage': 'duplicate', 'duplicate_of': duplicate_of}; db.commit(); db.refresh(document); return document
    try:
        document.processing_status = {'stage': 'extracting'}; db.commit(); extraction_service.extract_from_document(document, db)
...
async def upload_document(file: UploadFile = File(...), document_category: Optional[str] = Form(None), document_subcategory: Optional[str] = Form(None), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    profile = get_or_create_profile(current_user, db); name = file.filename or 'document'; content = await file.read()
    if not is_supported(name): raise HTTPException(400, 'Unsupported document format')
    return create_record(profile, name, content, db, document_category, document_subcategory)
...
async def bulk_upload(files: List[UploadFile] = File(...), document_category: Optional[str] = Form(None), document_subcategory: Optional[str] = Form(None), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    profile = get_or_create_profile(current_user, db); created = []; rejected = []
    for upload in files:
        name = upload.filename or 'document'
        try:
            content = await upload.read()
            if Path(name).suffix.lower() == '.zip':
                created.append(create_record(profile, name, content, db, 'other', 'archive', 'zip_upload', extract=False))
                for entry_name, entry_content in extract_zip(content): created.append(create_record(profile, entry_name, entry_content, db, document_category, document_subcategory, 'zip_extract', {'zip_source': name}))
            elif is_supported(name):
                created.append(create_record(profile, name, content, db, document_category, document_subcategory))
            else: rejected.append({'filename': name, 'reason': 'Unsupported format'})
        except Exception as exc: rejected.append({'filename': name, 'reason': str(exc)[:300]})
...
def upload_document_base64(request: DocumentUploadRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return create_record(get_or_create_profile(current_user, db), request.filename, content, db, request.document_category, request.document_subcategory)


@router.get('/', response_model=List[DocumentResponse])
def get_documents(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    profile = db.query(CandidateProfile).filter(CandidateProfile.user_id == current_user.id, CandidateProfile.is_active == True).first()
    return db.query(Document).filter(Document.candidate_id == profile.id).order_by(Document.created_at.desc()).all() if profile else []
...
def get_document(document_id: uuid.UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not document: raise HTTPException(404, 'Document not found')
    if not db.query(CandidateProfile).filter(CandidateProfile.id == document.candidate_id, CandidateProfile.user_id == current_user.id).first(): raise HTTPException(403, 'Not authorized')
    return document
