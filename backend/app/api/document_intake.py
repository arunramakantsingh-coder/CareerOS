...
def get_profile(current_user: User, db: Session) -> CandidateProfile:
    profile = (
        ...
        .filter(CandidateProfile.user_id == current_user.id, CandidateProfile.is_active == True)
        .first()
    )
    if not profile:
...
def parse_relative_paths(raw: Optional[str], file_count: int) -> list[Optional[str]]:
    ...
    try:
        ...
        paths = [str(x) if x is not None else None for x in decoded] if isinstance(decoded, list) else [str(decoded)]
...
def persist_document(
    ...
    """Persist a source document into Document Vault only.

    Upload/intake is deliberately non-mutating with respect to canonical career facts. Raw text,
    classification and vault metadata are stored here. Profile extraction/reconciliation must be
    invoked explicitly through the intelligence workflow.
    """
    if len(content) > MAX_FILE_SIZE:
        raise HTTPException(413, f"{filename}: file exceeds 25MB limit")
    ...
    duplicate = (
        db.query(Document)
        .filter(Document.candidate_id == profile.id, Document.content_hash == content_hash)
        .first()
    )
    ...
    source_metadata = {
        ...
        "deterministic_extraction": {
            "status": "not_run",
            "reason": "Deferred until explicit extraction/reconciliation; upload must not mutate the professional profile.",
        },
    }
    ...
    document = Document(
        ...
        processing_status={
            "stage": "vault_ingested",
            "classification": classification,
            "extraction": extraction_meta,
            "deterministic_extraction": "not_run",
        },
        ...
        detected_type=f"{classification['category']}:{classification['subcategory']}",
    ...
    document.processing_stage = "needs_review" if duplicate else "vault_ready"
    document.processing_status = {
        **(document.processing_status or {}),
        "stage": "duplicate_review" if duplicate else "vault_ready",
...
async def batch_upload(
    ...
    for index, upload in enumerate(files):
        ...
        if suffix == ".zip" or upload.content_type in {"application/zip", "application/x-zip-compressed"}:
            zip_doc = persist_document(profile, original, content, upload.content_type, db, batch_id, relative_path=rel)
            extracted = []
            try:
                ...
                zip_doc.processing_status = {**(zip_doc.processing_status or {}), "stage": "zip_extracted", "children": extracted}
                zip_doc.processing_stage = "complete"
                db.commit()
            except Exception as exc:
                zip_doc.status = "failed"
                zip_doc.processing_stage = "failed"
                zip_doc.processing_status = {**(zip_doc.processing_status or {}), "stage": "zip_failed", "error": str(exc)}
                db.commit()
                raise HTTPException(400, f"ZIP processing failed for {original}: {exc}")
            results.append({"id": str(zip_doc.id), "filename": zip_doc.original_filename, "status": zip_doc.status, "type": "zip", "children": extracted})
            continue
        ...
        results.append(
            {
                ...
                "display_name": document.user_label,
