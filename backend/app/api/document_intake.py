...
def persist_document(
    ...
    # Text is derived data, not the authoritative document.
    source_metadata["extracted_text"] = text[:100000]
...
async def batch_upload(
    ...
    """Bulk upload files, folders selected by the browser, and ZIP archives."""
    if not files:
    ...
    for index, upload in enumerate(files):
        ...
        rel = safe_relative_path((relative_paths or [None] * len(files))[index] if relative_paths and index < len(relative_paths) else None)
        suffix = Path(original).suffix.lower()

        if suffix == ".zip" or upload.content_type in {"application/zip", "application/x-zip-compressed"}:
            zip_doc = persist_document(
                profile, original, content, upload.content_type, db, batch_id,
                relative_path=rel, is_zip_content=False,
            )
            try:
                extracted = []
                for entry_name, entry_content in iter_zip_entries(content):
                    child = persist_document(
                        profile, entry_name, entry_content, None, db, batch_id,
                        relative_path=entry_name, is_zip_content=True, parent_zip_id=zip_doc.id,
                    )
                    extracted.append({"id": str(child.id), "filename": child.original_filename, "status": child.status})
            ...
            results.append({"id": str(zip_doc.id), "filename": zip_doc.original_filename, "status": zip_doc.status, "type": "zip"})
            continue
