...
def document_summary(doc: Document, candidate_id: UUID, db: Session) -> dict[str, Any]:
    ...
    return {"id": str(doc.id), "original_filename": doc.original_filename, "filename": doc.filename, "user_label": doc.user_label, "detected_type": detected, "category": doc.document_category, "subcategory": doc.document_subcategory, "issuer": doc.issuer, "issue_date": doc.issue_date, "expiry_date": doc.expiry_date, "document_number": doc.document_number, "classification_confidence": doc.classification_confidence or classification.get("confidence"), "classification_reason": doc.classification_reason or classification.get("reason"), "verification_status": doc.verification_status, "processing_stage": doc.processing_stage, "status": doc.status, "extraction_status": doc.extraction_status, "file_size": doc.file_size, "mime_type": doc.mime_type, "created_at": doc.created_at, "linked_fact_count": len(links)}
...
def identity_overview(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    def fact(item, fact_type): return {"id": str(item.id), **{k: getattr(item, k) for k in item.__table__.columns.keys() if k not in {"id", "candidate_id"}}, "evidence": fact_links(profile.id, fact_type, item.id, db)}
    section = {"personal": {"status": "complete" if profile.full_name and profile.title else "needs_review", "label": "Personal details & professional identity"}, "resume": {"status": "complete" if any(d.document_category == "cv" for d in documents) else "needs_input", "label": "Resume & career positioning"}, "employment": {"status": "complete" if experiences else "needs_input", "label": "Employment history"}, "education": {"status": "complete" if educations else "needs_input", "label": "Education"}, "certifications": {"status": "complete" if certifications else "pending", "label": "Certifications & credentials", "count": len(certifications)}, "skills": {"status": "complete" if skills else "needs_input", "label": "IT skills & key skills", "count": len(skills)}, "preferences": {"status": "needs_review" if not profile.work_preferences else "complete", "label": "Career profile & preferences"}, "sources": {"status": "complete" if documents else "needs_input", "label": "Profile sources & next actions"}}
    evidence_count = db.query(CareerFactEvidence).filter(CareerFactEvidence.candidate_id == profile.id).count()
    return {"profile": {"id": str(profile.id), "full_name": profile.full_name, "location": profile.location, "title": profile.title, "summary": profile.summary, "primary_email": profile.primary_email, "primary_phone": profile.primary_phone, "linkedin_url": profile.linkedin_url, "years_experience": profile.years_experience, "industries": profile.industries or [], "seniority": profile.seniority, "work_preferences": profile.work_preferences or {}, "completeness_score": profile.completeness_score or 0}, "experiences": [fact(x, "employment") for x in experiences], "educations": [fact(x, "education") for x in educations], "certifications": [fact(x, "certification") for x in certifications], "skills": [fact(x, "skill") for x in skills], "documents": [document_summary(x, profile.id, db) for x in documents], "personas": [{"id": str(x.id), "name": x.name, "description": x.description, "positioning": x.positioning, "is_active": x.is_active, "target_titles": x.target_titles or []} for x in db.query(Persona).filter(Persona.user_id == user.id).all()], "persona_suggestions": [{"id": str(x.id), "name": x.name, "role_family": x.role_family, "positioning": x.positioning, "target_titles": x.target_titles or [], "confidence": x.confidence, "reason": x.reason, "missing_evidence": x.missing_evidence or [], "supporting_document_ids": x.supporting_document_ids or []} for x in suggestions], "section_navigation": section, "intelligence": {"profile_completeness": profile.completeness_score or 0, "evidence_coverage": round(min(100, evidence_count * 10), 1), "data_confidence": round(sum((x.confidence or 0) for x in certifications + educations + skills) / max(1, len(certifications) + len(educations) + len(skills)) * 100, 1), "persona_readiness": round(min(100, (len(experiences) * 20 + len(skills) * 5 + len(certifications) * 10)), 1)}, "next_actions": _next_actions(profile, documents, experiences, educations, certifications, suggestions)}
...
def evidence_library(user: User = Depends(get_current_user), db: Session = Depends(get_db), category: str | None = Query(None), verification: str | None = Query(None), search: str | None = Query(None)):
    profile = profile_for(user, db); docs = db.query(Document).filter(Document.candidate_id == profile.id).order_by(Document.created_at.desc()).all(); result=[]
    for doc in docs:
        ...
        links=db.query(CareerFactEvidence).filter(CareerFactEvidence.candidate_id==profile.id,CareerFactEvidence.document_id==doc.id).all(); item["linked_facts"]=[{"fact_type":x.fact_type,"fact_id":str(x.fact_id),"relationship":x.relationship,"confidence":x.confidence} for x in links]; result.append(item)
...
def document_detail(document_id: UUID, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    item=document_summary(doc,profile.id,db); item["source_metadata"]={k:v for k,v in (doc.source_metadata or {}).items() if k not in {"extracted_text"}}; item["linked_facts"]=[]
...
def reclassify_document(document_id: UUID, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    path=Path(doc.storage_path).resolve()
    if STORAGE_ROOT not in path.parents or not path.is_file(): raise HTTPException(404,"Stored document is unavailable")
    text,meta=extract_text(doc.original_filename,doc.mime_type,path.read_bytes()); result=classify_document(doc.original_filename,text)
...
def generate_persona_suggestions(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    profile=profile_for(user,db); experiences=db.query(ProfessionalExperience).filter(ProfessionalExperience.candidate_id==profile.id).all(); skills=db.query(CandidateSkill).filter(CandidateSkill.candidate_id==profile.id).all(); certs=db.query(CandidateCertification).filter(CandidateCertification.candidate_id==profile.id).all(); docs=db.query(Document).filter(Document.candidate_id==profile.id,Document.document_category=="cv").all()
    ...
    patterns=[("Cloud & Infrastructure",["cloud","aws","azure","gcp","infrastructure","kubernetes","vmware"],["Cloud Architect","Cloud Infrastructure Architect","Infrastructure Architect"]),("Cybersecurity",["security","cybersecurity","siem","zero trust","incident response","cissp","cism","cisa"],["Cybersecurity Architect","Cloud Security Architect","Security Transformation Leader"]),("Network & Connectivity",["network","routing","switching","bgp","sd-wan","mpls","firewall"],["Network Architect","Network & Security Architect","Connectivity Leader"]),("Technology Leadership",["director","vp","head of","leader","leadership","governance","transformation","manager"],["Enterprise Technology Leader","Technology Transformation Leader","Infrastructure & Operations Leader"]),("Architecture",["architect","architecture","enterprise architecture","solution architecture"],["Enterprise Architect","Solution Architect","Technology Architect"])]
    existing={s.name for s in db.query(PersonaSuggestion).filter(PersonaSuggestion.user_id==user.id,PersonaSuggestion.status=="suggested").all()}; created=[]
    for family,terms,titles in patterns:
        hits=[t for t in terms if t in corpus]
        if not hits: continue
        confidence=min(0.94,0.50+0.08*len(hits)+0.03*min(len(experiences),5)+0.02*min(len(certs),4)); name=titles[0]
        if name in existing: continue
        supporting_docs=[str(x.id) for x in docs]; reason=f"Suggested because your career evidence contains signals for {family}: {', '.join(hits[:5])}."; missing=[]
        ...
        suggestion=PersonaSuggestion(id=uuid4(),user_id=user.id,candidate_id=profile.id,name=name,role_family=family,positioning=f"Position the same Career Vault evidence around {name.lower()} outcomes without changing the underlying facts.",target_titles=titles,supporting_fact_ids=[str(x.id) for x in experiences+skills+certs],supporting_document_ids=supporting_docs,missing_evidence=missing,confidence=confidence,reason=reason,status="suggested"); db.add(suggestion); created.append(suggestion); existing.add(name)
    db.commit(); return [{"id":str(x.id),"name":x.name,"role_family":x.role_family,"positioning":x.positioning,"target_titles":x.target_titles or [],"confidence":x.confidence,"reason":x.reason,"missing_evidence":x.missing_evidence or [],"supporting_document_ids":x.supporting_document_ids or []} for x in created]


@router.get("/personas/suggestions")
def persona_suggestions(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    rows=db.query(PersonaSuggestion).filter(PersonaSuggestion.user_id==user.id).order_by(PersonaSuggestion.confidence.desc()).all(); return [{"id":str(x.id),"name":x.name,"role_family":x.role_family,"positioning":x.positioning,"target_titles":x.target_titles or [],"confidence":x.confidence,"reason":x.reason,"missing_evidence":x.missing_evidence or [],"status":x.status,"supporting_document_ids":x.supporting_document_ids or []} for x in rows]
...
def ensure_career_profile(user: User, candidate: CandidateProfile, db: Session) -> CareerProfile:
    ...
    if not cp: cp=CareerProfile(user_id=user.id,is_active=True); db.add(cp)
    cp.name=candidate.full_name or cp.name; cp.description=candidate.summary or cp.description; cp.seniority=candidate.seniority or cp.seniority; cp.years_experience=int(candidate.years_experience) if candidate.years_experience else cp.years_experience; cp.preferred_locations=(candidate.work_preferences or {}).get("preferred_locations") or cp.preferred_locations; cp.remote_preference=(candidate.work_preferences or {}).get("remote_preference") or cp.remote_preference; cp.target_roles=(candidate.work_preferences or {}).get("target_roles") or cp.target_roles; cp.industries=candidate.industries or cp.industries; db.flush(); return cp
...
def connection_diagnostics(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    rows=db.query(EmailConnectorAccount).filter(EmailConnectorAccount.user_id==user.id).all(); external=[]
    for x in db.query(__import__("app.models.external_identity",fromlist=["ExternalIdentity"]).ExternalIdentity).filter(__import__("app.models.external_identity",fromlist=["ExternalIdentity"]).ExternalIdentity.user_id==user.id, __import__("app.models.external_identity",fromlist=["ExternalIdentity"]).ExternalIdentity.is_active.is_(True)).all(): external.append({"id":str(x.id),"provider":x.provider,"provider_email":x.provider_email,"scopes":x.scopes or [],"token_status":"present" if x.access_token else "missing","token_expires_at":x.token_expires_at,"last_used":x.last_used})
