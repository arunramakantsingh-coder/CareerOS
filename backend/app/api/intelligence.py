...
def _ensure_catalog_rows(db: Session) -> None:
    ...
    if changed:
        ...
        if not preferred: preferred = db.query(IntelligenceProviderConfig).filter(IntelligenceProviderConfig.provider == "ollama").first()
        if preferred and not db.query(IntelligenceProviderConfig).filter(IntelligenceProviderConfig.active.is_(True)).first():
            preferred.active = True; _set_lifecycle(preferred, "active"); preferred.routing_policy = {**(preferred.routing_policy or {}), "operator_primary": True}
        db.commit()


def _configuration_error(row: IntelligenceProviderConfig) -> str | None:
    try: validate_provider_configuration(row.provider, row.model, row.base_url)
    except ProviderConfigurationError as exc: return str(exc)
    if row.configured and row.provider != "ollama" and not row.encrypted_api_key: return "API credential is not configured"
    return None


def _public_provider(row: IntelligenceProviderConfig) -> dict[str, Any]:
    metadata = row.metadata_json or {}; telemetry = metadata.get("telemetry") or {}; policy = row.routing_policy or {}; health = metadata.get("health") or {}
...
def _health_is_fresh(row: IntelligenceProviderConfig, db: Session) -> bool:
    rows = db.query(IntelligenceProviderConfig).all(); return health_is_fresh(row, health_ttl_seconds(policy_from_rows(rows)))


def _health_rank_key(row: IntelligenceProviderConfig) -> tuple[float, float, float, float, int, int, str]:
    health = (row.metadata_json or {}).get("health") or {}; checks = int(health.get("checks") or 0); successful = int(health.get("successful_checks") or 0); reliability = successful / checks if checks else 0.0; recent_failures = int(health.get("consecutive_failures") or 0); p95 = float(health.get("p95_latency_ms") or 999999.0); avg = float(health.get("avg_latency_ms") or 999999.0); preferred = bool((row.routing_policy or {}).get("operator_primary")); return (-reliability, recent_failures, p95, avg, 0 if preferred else 1, int(row.priority), row.label)
...
async def _call(path: str, method: str = "GET", payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    except httpx.HTTPError as exc: raise HTTPException(status_code=503, detail=f"Intelligence Engine unavailable: {exc}") from exc
...
async def providers(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    _ensure_catalog_rows(db); rows = db.query(IntelligenceProviderConfig).order_by(IntelligenceProviderConfig.priority, IntelligenceProviderConfig.label).all(); active = _primary_active(rows, db)
...
async def observability(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    _ensure_catalog_rows(db); rows = db.query(IntelligenceProviderConfig).order_by(IntelligenceProviderConfig.priority, IntelligenceProviderConfig.label).all(); configured = [row for row in rows if row.configured]; active = [row for row in configured if row.active]; healthy = [row for row in active if _health_is_fresh(row, db)]
    total_requests = total_tokens = total_failures = total_fallbacks = 0
    for row in rows:
        telemetry = (row.metadata_json or {}).get("telemetry") or {}; total_requests += int(telemetry.get("requests") or 0); total_tokens += int(telemetry.get("total_tokens") or 0); total_failures += int(telemetry.get("failed_requests") or 0); total_fallbacks += int(telemetry.get("fallback_requests") or 0)
...
async def routing_preview(task_type: str = Query(default="profile_reconciliation", min_length=2, max_length=80), db: Session = Depends(get_db), _: User = Depends(require_developer)):
    ...
    for row in rows:
        ...
        elif not all(cap in (set(row.capabilities or []) | set((PROVIDER_CATALOG.get(row.provider) or {}).get("capabilities") or [])) for cap in requirements): reason = f"missing_capabilities: {', '.join(cap for cap in requirements if cap not in (set(row.capabilities or []) | set((PROVIDER_CATALOG.get(row.provider) or {}).get("capabilities") or [])))}"
    ...
    return {"task_type": task_type, "required_capabilities": requirements, "health_ttl_seconds": ttl, "candidates": [{"provider": row.provider, "model": row.model, "rank": index + 1, "reason": "eligible"} for index, row in enumerate(ranked)], "excluded_candidates": excluded, "selected_provider": ranked[0].provider if ranked else None, "selected_model": ranked[0].model if ranked else None, "ranking_reason": "reliability → recent failures → p95 latency → average latency → operator preference → manual priority"}
...
def _save_row(db: Session, request: ProviderSaveRequest) -> IntelligenceProviderConfig:
    ...
    if request.api_key: secret = request.api_key.strip(); row.encrypted_api_key = encrypt_secret(secret); row.api_key_last4 = secret[-4:]
    row.configured = True if name == "ollama" else bool(row.encrypted_api_key)
    _set_lifecycle(row, "active" if row.active else "configured")
    row.last_error = None
...
async def activate_provider(request: ProviderActivateRequest, db: Session = Depends(get_db), _: User = Depends(require_developer)):
    ...
    row.active = True; _set_lifecycle(row, "active"); row.last_error = None; db.commit(); active_rows = db.query(IntelligenceProviderConfig).all(); primary = _primary_active(active_rows, db)
...
async def deactivate_provider(request: ProviderActivateRequest, db: Session = Depends(get_db), _: User = Depends(require_developer)):
    name = request.provider.strip().lower(); _ensure_catalog_rows(db); row = db.query(IntelligenceProviderConfig).filter(IntelligenceProviderConfig.provider == name).first()
    if not row: raise HTTPException(status_code=404, detail=f"Provider not found: {name}")
    row.active = False; _set_lifecycle(row, "deactivated" if row.configured else "not_configured"); row.routing_policy = {**(row.routing_policy or {}), "operator_primary": False}; db.commit(); db.refresh(row); active_rows = db.query(IntelligenceProviderConfig).all(); primary = _primary_active(active_rows, db)
...
async def test_provider(request: ProviderSaveRequest, db: Session = Depends(get_db), _: User = Depends(require_developer)):
    ...
    try:
        ...
        if request.model or request.base_url:
            payload.update(validate_provider_configuration(name, request.model or row.model, request.base_url if request.base_url is not None else row.base_url))
...
async def retrieve(request: RetrievalRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)): return retrieve_career_knowledge(db=db, user_id=current_user.id, query=request.query, top_k=request.top_k, filters=request.filters)
