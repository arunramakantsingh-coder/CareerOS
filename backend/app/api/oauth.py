...
def friendly_oauth_error(message: str, provider: str) -> str:
    raw = (message or "OAuth authorization failed").strip()
    if provider == "google" and raw.lower() in {"access_denied", "the user denied access"}:
        return "Google blocked this mailbox authorization. If CareerOS is in Google OAuth Testing mode, add this Google account under Google Cloud Console → Google Auth Platform → Audience → Test users, then retry. Google SSO and Gmail mailbox access are separate permissions."
    return raw[:300]
...
def oauth_error(frontend: str, provider: str, message: str, destination: str = "login") -> RedirectResponse:
    ...
    return RedirectResponse(f"{frontend}{path}?{urlencode({'oauth': 'error', 'provider': provider, 'message': friendly_oauth_error(message, provider)})}")
...
async def google_gmail_authorize_url(request: Request, current_user: User = Depends(get_current_user)):
    return {"authorization_url": gmail_authorization_url(request, current_user)}
...
async def google_callback(request: Request, code: str | None = None, state: str | None = None, error: str | None = None, error_description: str | None = None, db: Session = Depends(get_db)):
    if not state:
    ...
    if error:
        return oauth_error(frontend, "google", error_description or error, destination)
    if not code:
...
async def linkedin_callback(request: Request, code: str | None = None, state: str | None = None, error: str | None = None, error_description: str | None = None, db: Session = Depends(get_db)):
    if not state:
        return oauth_error(frontend_origin(request), "linkedin", "Missing OAuth state")
    payload = parse_state(state, "linkedin")
    frontend = payload.get("frontend") or frontend_origin(request)
    if error:
        return oauth_error(frontend, "linkedin", error_description or error)
    if not code:
