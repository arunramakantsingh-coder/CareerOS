...
def callback_uri(request: Request, provider: str) -> str: return f"{request.url.scheme}://{request.url.netloc}/api/v1/auth/oauth/{provider}/callback"
def state_token(provider: str, action: str, request: Request, user_id: str | None = None) -> str: return create_access_token({"purpose":"oauth_state","provider":provider,"action":action,"user_id":user_id,"frontend":frontend_origin(request)}, expires_delta=timedelta(minutes=10))
...
def oauth_error(frontend: str, provider: str, message: str) -> RedirectResponse: return RedirectResponse(f"{frontend}/settings?{urlencode({'oauth':'error','provider':provider,'message':message[:200]})}")
def oauth_complete_html(token: str, frontend: str, provider: str) -> HTMLResponse:
    safe_frontend=frontend.replace("'","%27");safe_token=token.replace("'","%27")
...
def gmail_authorization_url(request: Request, user: User) -> str:
    ...
    redirect_uri=callback_uri(request,"google");scopes=["openid","email","profile","https://www.googleapis.com/auth/gmail.readonly"]
...
async def exchange_code(provider: str, code: str, redirect_uri: str) -> dict:
    if provider=="google": data={"code":code,"client_id":settings.GOOGLE_CLIENT_ID,"client_secret":settings.GOOGLE_CLIENT_SECRET,"redirect_uri":redirect_uri,"grant_type":"authorization_code"};token_url=GOOGLE_TOKEN
    else: data={"code":code,"client_id":settings.LINKEDIN_CLIENT_ID,"client_secret":settings.LINKEDIN_CLIENT_SECRET,"redirect_uri":redirect_uri,"grant_type":"authorization_code"};token_url=LINKEDIN_TOKEN
    async with httpx.AsyncClient(timeout=15) as client:
        response=await client.post(token_url,data=data,headers={"Accept":"application/json"});response.raise_for_status();return response.json()
...
async def fetch_userinfo(provider: str, access_token: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        response=await client.get(endpoint,headers={"Authorization":f"Bearer {access_token}"});response.raise_for_status();return response.json()
...
def upsert_identity(db: Session, user: User, provider: str, info: dict, tokens: dict, scopes: list[str]) -> ExternalIdentity:
    ...
    identity.provider_email=info.get("email");identity.access_token=tokens.get("access_token");identity.refresh_token=tokens.get("refresh_token") or identity.refresh_token;identity.provider_data=info;identity.scopes=scopes;identity.last_used=datetime.utcnow();db.commit();db.refresh(identity);return identity
...
async def google_gmail_start(request: Request,current_user: User=Depends(get_current_user)): return RedirectResponse(gmail_authorization_url(request,current_user))

@router.post("/google/gmail/authorize-url")
async def google_gmail_authorize_url(request: Request,current_user: User=Depends(get_current_user)): return {"authorization_url":gmail_authorization_url(request,current_user)}
...
async def google_callback(request: Request,code: str|None=None,state: str|None=None,error: str|None=None,db: Session=Depends(get_db)):
    ...
    payload=parse_state(state,"google");frontend=payload.get("frontend") or frontend_origin(request)
    if error:return oauth_error(frontend,"google",error)
    if not code:return oauth_error(frontend,"google","Missing authorization code")
    ...
    except Exception as exc:return oauth_error(frontend,"google",f"Google authorization failed: {exc}")
    if payload.get("action")=="gmail":
        user=db.query(User).filter(User.id==payload.get("user_id"),User.is_active==True).first()
        if not user:return oauth_error(frontend,"google","CareerOS session expired; please sign in again")
        upsert_identity(db,user,"google",info,tokens,["openid","email","profile","https://www.googleapis.com/auth/gmail.readonly"]);return RedirectResponse(f"{frontend}/settings?oauth=connected&provider=gmail")
    email=info.get("email")
    if not email:return oauth_error(frontend,"google","Google did not return an email address")
    user=db.query(User).filter(User.email==email).first()
    if not user: tenant=Tenant(name="default",plan="free",status="active");db.add(tenant);db.flush();user=User(email=email,name=info.get("name") or email.split("@")[0],tenant_id=tenant.id,is_active=True);db.add(user);db.flush()
    upsert_identity(db,user,"google",info,tokens,["openid","profile","email"]);enrich_candidate_from_identity(db,user,info);return oauth_complete_html(create_access_token({"sub":str(user.id),"tenant_id":str(user.tenant_id)}),frontend,"Google")
...
async def linkedin_callback(request: Request,code: str|None=None,state: str|None=None,error: str|None=None,db: Session=Depends(get_db)):
    ...
    if error:return oauth_error(frontend,"linkedin",error)
    ...
    except Exception as exc:return oauth_error(frontend,"linkedin",f"LinkedIn authorization failed: {exc}")
    email=info.get("email")
    if not email:return oauth_error(frontend,"linkedin","LinkedIn did not return an email address")
    user=db.query(User).filter(User.email==email).first()
    if not user:tenant=Tenant(name="default",plan="free",status="active");db.add(tenant);db.flush();user=User(email=email,name=info.get("name") or email.split("@")[0],tenant_id=tenant.id,is_active=True);db.add(user);db.flush()
    upsert_identity(db,user,"linkedin",info,tokens,["openid","profile","email"]);enrich_candidate_from_identity(db,user,info);return oauth_complete_html(create_access_token({"sub":str(user.id),"tenant_id":str(user.tenant_id)}),frontend,"LinkedIn")
