...
SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        "profile": {
            ...
            "required": ["full_name", "location", "title", "summary", "primary_email", "primary_phone", "linkedin_url", "years_experience", "seniority", "industries"],
        },
        "experiences": EMPLOYMENT_SCHEMA["properties"]["experiences"],
        "skills": {
            "type": "array", "maxItems": 120,
            "items": {"type": "object", "additionalProperties": False, "properties": {"name": {"type": "string"}, "category": {"type": "string"}, "proficiency": {"type": ["string", "null"]}}, "required": ["name", "category", "proficiency"]},
        },
        "certifications": {
            "type": "array", "maxItems": 50,
            "items": {"type": "object", "additionalProperties": False, "properties": {"name": {"type": "string"}, "issuer": {"type": ["string", "null"]}, "issue_date": {"type": ["string", "null"]}, "expiry_date": {"type": ["string", "null"]}, "credential_reference": {"type": ["string", "null"]}}, "required": ["name", "issuer", "issue_date", "expiry_date", "credential_reference"]},
        },
        "education": {
            "type": "array", "maxItems": 20,
            "items": {"type": "object", "additionalProperties": False, "properties": {"institution": {"type": "string"}, "degree": {"type": "string"}, "field_of_study": {"type": ["string", "null"]}, "start_date": {"type": ["string", "null"]}, "end_date": {"type": ["string", "null"]}, "grade": {"type": ["string", "null"]}}, "required": ["institution", "degree", "field_of_study", "start_date", "end_date", "grade"]},
        },
        "projects": {
            "type": "array", "maxItems": 40,
            "items": {"type": "object", "additionalProperties": False, "properties": {"name": {"type": "string"}, "description": {"type": ["string", "null"]}, "role": {"type": ["string", "null"]}, "technologies": {"type": "array", "items": {"type": "string"}, "maxItems": 30}, "responsibilities": {"type": "array", "items": {"type": "string"}, "maxItems": 20}, "achievements": {"type": "array", "items": {"type": "string"}, "maxItems": 20}, "client": {"type": ["string", "null"]}, "start_date": {"type": ["string", "null"]}, "end_date": {"type": ["string", "null"]}}, "required": ["name", "description", "role", "technologies", "responsibilities", "achievements", "client", "start_date", "end_date"]},
        },
        "accomplishments": {
            "type": "array", "maxItems": 50,
            "items": {"type": "object", "additionalProperties": False, "properties": {"title": {"type": "string"}, "description": {"type": ["string", "null"]}, "category": {"type": ["string", "null"]}, "date": {"type": ["string", "null"]}, "metrics": {"type": "array", "items": {"type": "string"}, "maxItems": 10}}, "required": ["title", "description", "category", "date", "metrics"]},
        },
    },
...
- Extract skills from explicit Skills, Technical Skills, Core Competencies and technology content; classify each as Technical/IT, Leadership, Architecture, Security, Networking, Cloud, Soft Skill, or Other as appropriate.
- Extract certifications and education only when stated in the CV.
- Extract named projects only when the CV explicitly presents them as projects, programmes, implementations or major engagements; do not invent project names.
- Extract accomplishments only when there is a concrete achievement, recognition, publication, award, measurable outcome or explicit accomplishment statement.
...
def _persist(document: Document, profile: CandidateProfile, payload: dict[str, Any], db: Session) -> dict[str, int]:
    ...
    projects = _merge_objects(profile.projects or [], payload.get("projects") or [], "name")
    accomplishments = _merge_objects(profile.accomplishments or [], payload.get("accomplishments") or [], "title")
...
def _skill(doc: Document, profile: CandidateProfile, skill_name: Any, category: Any, proficiency: Any, db: Session) -> None:
    ...
    if not row:
        row = CandidateSkill(candidate_id=profile.id, name=name, source_type="cv_ai", source_id=doc.id)
        db.add(row)
    row.category = row.category or _clean(category) or "Technical/IT"
    row.proficiency = row.proficiency or _clean(proficiency)
...
def _cert(doc: Document, profile: CandidateProfile, item: dict[str, Any], db: Session) -> None:
    ...
    if not row:
        row = CandidateCertification(candidate_id=profile.id, name=name, issuer=issuer, source_type="cv_ai", source_id=doc.id); db.add(row)
    row.issuer = row.issuer if row.issuer != "Unknown" else issuer
    row.issue_date = row.issue_date or _date(item.get("issue_date")); row.expiry_date = row.expiry_date or _date(item.get("expiry_date")); row.credential_reference = row.credential_reference or _clean(item.get("credential_reference")); row.confidence = max(float(row.confidence or 0), 0.85)
...
def _edu(doc: Document, profile: CandidateProfile, item: dict[str, Any], db: Session) -> None:
    ...
    if not row:
        row = CandidateEducation(candidate_id=profile.id, institution=institution, degree=degree, source_type="cv_ai", source_id=doc.id); db.add(row)
...
def _merge_objects(existing: list[Any], incoming: list[Any], key_field: str) -> list[dict[str, Any]]:
    ...
    for item in incoming:
        ...
        obj = dict(item); obj["id"] = str(uuid4()); obj["source_document_id"] = str(_CURRENT_DOCUMENT_ID) if _CURRENT_DOCUMENT_ID else None
        result.append(obj); seen.add(key)
    return result


_CURRENT_DOCUMENT_ID: UUID | None = None
...
def _evidence(candidate_id: Any, doc: Document, kind: str, fact_id: Any, confidence: float, excerpt: Any, db: Session) -> None:
    ...
    if row:
        row.confidence = max(row.confidence, confidence); row.excerpt = _clean(excerpt) or row.excerpt; return
