...
async def lifespan(app: FastAPI):
    logger = logging.getLogger(__name__); logger.info(f'Starting CareerOS API in {settings.ENVIRONMENT} mode'); yield; logger.info('Shutting down CareerOS API')

app = FastAPI(title='CareerOS API', version='0.1.0', description='AI-Powered Global Career Operating System', lifespan=lifespan)

cors_kwargs = dict(allow_origins=settings.ALLOWED_ORIGINS, allow_credentials=True, allow_methods=['*'], allow_headers=['*'])
if settings.ENVIRONMENT == 'development':
    cors_kwargs['allow_origin_regex'] = r'^https?://(localhost|127\.0\.0\.1|10\.\d{1,3}\.\d{1,3}|172\.1[6-9]\.\d{1,3}\.\d{1,3}|172\.2\d\.\d{1,3}\.\d{1,3}|172\.3[0-1]\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3})(:\d+)?$'
...
@app.get('/')
async def root(): return {'name': 'CareerOS API', 'version': '0.1.0', 'status': 'operational', 'environment': settings.ENVIRONMENT}
