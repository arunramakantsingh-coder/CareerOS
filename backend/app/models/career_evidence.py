...
class CareerEvidence(Base, TimestampMixin):
    ...
    source_type = Column(String(50), nullable=False)  # Employment, Project, Certification, Skill, Education, File, URL
    
    # Reference to the source entity
    source_id = Column(UUID(as_uuid=True), nullable=True)
    source_entity = Column(String(50), nullable=True)  # The entity type (Employment, Project, etc.)
    
    # Evidence details
    source_file = Column(String(500), nullable=True)  # File path or URL
    excerpt = Column(Text, nullable=True)  # Relevant excerpt from source
    confidence = Column(Float, default=1.0)  # 0.0 to 1.0
    
    verified_at = Column(DateTime, nullable=True)
    verified_by = Column(String(100), nullable=True)  # AI or Human
