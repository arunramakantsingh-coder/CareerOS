...
def row(name: str, *, active: bool = True, configured: bool = True, capabilities=None, checked_minutes_ago: int = 1, priority: int = 100, reliability=(9, 10), p95=100.0, avg=50.0, operator_primary=False, model="model"):
    ...
    return SimpleNamespace(
        ...
        encrypted_api_key="encrypted" if name != "ollama" else None,
        priority=priority,
        capabilities=capabilities or ["structured_output", "reasoning"],
        routing_policy={"fallback_enabled": True, "daily_request_limit": None, "operator_primary": operator_primary},
        metadata_json={"health": {"status": "healthy", "checked_at": checked, "successful_checks": successful, "checks": checks, "consecutive_failures": 0, "p95_latency_ms": p95, "avg_latency_ms": avg, "quota": {}}},
    )


def test_deactivated_provider_never_selected(monkeypatch):
    engine = RoutedIntelligenceEngine()
    rows = [row("openrouter", active=False), row("ollama")]
    class FakeDB:
...
def test_stale_provider_never_selected(monkeypatch):
    engine = RoutedIntelligenceEngine()
    rows = [row("openrouter", checked_minutes_ago=60), row("ollama")]
    class FakeDB:
        def query(self, _): return SimpleNamespace(all=lambda: rows)
        def close(self): pass
    monkeypatch.setattr("app.intelligence.engine_runtime.SessionLocal", lambda: FakeDB())
...
def test_unhealthy_provider_never_selected(monkeypatch):
    engine = RoutedIntelligenceEngine()
    bad = row("openrouter")
    bad.metadata_json["health"]["status"] = "unhealthy"
    rows = [bad, row("ollama")]
    class FakeDB:
        def query(self, _): return SimpleNamespace(all=lambda: rows)
        def close(self): pass
...
def test_incompatible_provider_excluded_without_stopping_routing(monkeypatch):
    engine = RoutedIntelligenceEngine()
    incompatible = row("openrouter", capabilities=["structured_output"])
    compatible = row("ollama")
...
def test_multiple_active_providers_are_dynamically_ranked(monkeypatch):
    engine = RoutedIntelligenceEngine()
    slower = row("openrouter", reliability=(8, 10), p95=500, avg=300)
    faster = row("ollama", reliability=(10, 10), p95=20, avg=10)
    rows = [slower, faster]
    class FakeDB:
        def query(self, _): return SimpleNamespace(all=lambda: rows)
        def close(self): pass
...
def test_manual_priority_cannot_override_health(monkeypatch):
    engine = RoutedIntelligenceEngine()
    bad = row("openrouter", checked_minutes_ago=60, priority=1)
    good = row("ollama", priority=1000)
    rows = [bad, good]
    class FakeDB:
        def query(self, _): return SimpleNamespace(all=lambda: rows)
        def close(self): pass
...
def test_provider_configuration_isolation():
    with pytest.raises(ProviderConfigurationError):
        validate_provider_configuration("openrouter", "gemma3:4b", "http://host.docker.internal:11434")
    with pytest.raises(ProviderConfigurationError):
        validate_provider_configuration("gemini", "gemma3:4b", "")
...
@pytest.mark.asyncio
async def test_failed_primary_falls_back_to_next_eligible(monkeypatch):
    engine = RoutedIntelligenceEngine()
    primary = row("openrouter", operator_primary=True)
    fallback = row("ollama")
    monkeypatch.setattr(engine, "_candidate_snapshot", lambda task, trace: ([primary, fallback], []))
    monkeypatch.setattr(engine, "_gateway_config", lambda item: {"provider": item.provider, "model": item.model, "base_url": item.base_url, "api_key": "key"})
    monkeypatch.setattr(engine, "_record_telemetry", lambda *args, **kwargs: None)
    ...
    class Client:
        ...
        async def post(self, *_args, **_kwargs):
            self.calls += 1
            return Response(self.calls > 1, "ollama" if self.calls > 1 else "openrouter")
    ...
    assert result["routing"]["attempts"][0]["status"] == "failed"
    assert result["routing"]["attempts"][1]["status"] == "success"
