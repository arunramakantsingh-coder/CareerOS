...
class FakeConnection:
    ...
    def execute(self, statement, params=None): self.executed.append((str(statement), params)); return None


M02_TABLES = {"external_identities", "candidate_profiles", "professional_experiences", "candidate_skills", "candidate_certifications", "candidate_educations", "documents", "extraction_results", "extraction_fields"}
DOCUMENT_015_COLUMNS = {"content_hash", "issuer", "issue_date", "expiry_date", "document_number", "document_type", "batch_id", "is_zip_content", "parent_zip_id", "classification_confidence"}


def test_detects_016_schema(monkeypatch):
    tables = M02_TABLES | {"users", "career_fact_evidence", "persona_suggestions", "email_connector_accounts"}
    ...
    assert migrations._detect_legacy_baseline() == "016_m02_identity_intelligence"
...
def test_normalizes_legacy_017_without_rerunning_schema_migration(monkeypatch):
    ...
    monkeypatch.setattr(migrations, "engine", SimpleNamespace(connect=lambda: connection, begin=lambda: connection))
    monkeypatch.setattr(migrations, "_get_version_rows", lambda: ["017_m02_employment_semantic", "018_m02_profile_sections"])
    monkeypatch.setattr(migrations, "_has_columns", lambda table, columns: True)
    monkeypatch.setattr(migrations, "_has_tables", lambda tables: True)
    monkeypatch.setattr(migrations, "_alembic_config", lambda: SimpleNamespace())
    calls = []
    monkeypatch.setattr(migrations.command, "upgrade", lambda cfg, revision: calls.append(("upgrade", revision)))
    migrations.reconcile_database()
