...
const PROVIDER_CATALOG: Provider[] = [
  ...
  { provider: 'openrouter', label: 'OpenRouter', category: 'cloud', model: 'openrouter/free', base_url: 'https://openrouter.ai/api/v1', configured: false, active: false, priority: 100, capabilities: ['routing', 'model_choice', 'structured_output', 'reasoning'] },
...
function healthLabel(p: Provider, fresh: boolean) {
  if (!p.configured) return 'NOT CHECKED';
...
function healthTone(p: Provider, fresh: boolean) {
  const label = healthLabel(p, fresh);
  if (label === 'HEALTHY') return 'good';
  if (label === 'UNHEALTHY') return 'warn';
  if (label === 'STALE') return 'blue';
  return 'muted';
}
function lifecycleLabel(p: Provider) {
  if (!p.configured) return 'NOT CONFIGURED';
  return p.active ? 'ACTIVE' : 'DEACTIVATED';
}
...
export default function IntelligenceEngine() {
  ...
  const [taskType, setTaskType] = useState('profile_reconciliation');
  ...
  const mergeProviders = (remote: Provider[], healthData?: any) => {
    ...
    return PROVIDER_CATALOG.map((fallback) => {
      ...
      return { ...fallback, ...(live || {}), health: h?.health || live?.health || fallback.health, health_fresh: h ? Boolean(h.health_fresh) : Boolean(live?.health_fresh) };
    });
  };
  ...
  const applySelected = (list: Provider[]) => {
    ...
    const catalog = PROVIDER_CATALOG.find((x) => x.provider === p.provider) || p;
    const invalid = p.configuration_valid === false;
    setForm({ model: invalid ? catalog.model || '' : p.model || '', base_url: invalid ? catalog.base_url || '' : p.base_url || '', api_key: '', priority: p.priority || 100 });
  };
  ...
  const load = async () => {
    ...
    const results = await Promise.allSettled([
      ...
      apiClient.intelligenceRoutingPreview(taskType),
      apiClient.intelligenceRuntimeTraces(10),
    ]);
    ...
    if (registryResult.status === 'fulfilled') {
      ...
      const base = mergeProviders(Array.isArray(data.providers) ? data.providers : []);
      setProviders(base); applySelected(base);
    ...
    if (healthResult.status === 'fulfilled') {
      const data = healthResult.value; setHealthError('');
    ...
    if (tracesResult.status === 'fulfilled') {
      const trace = (tracesResult.value.traces || []).find((x: any) => x.task_type === taskType);
      setLatestTrace(trace || null);
    }
    loadingRef.current = false;
  };

  useEffect(() => { void load(); }, [taskType]);
  useEffect(() => { const timer = window.setInterval(() => void load(), 15000); return () => window.clearInterval(timer); }, [taskType]);

  const selectProvider = (name: string) => {
    const p = providers.find((x) => x.provider === name) || PROVIDER_CATALOG.find((x) => x.provider === name);
    if (!p) return;
    setSelected(name);
    const catalog = PROVIDER_CATALOG.find((x) => x.provider === name) || p;
    const invalid = p.configuration_valid === false;
    setForm({ model: invalid ? catalog.model || '' : p.model || '', base_url: invalid ? catalog.base_url || '' : p.base_url || '', api_key: '', priority: p.priority || 100 });
    setMessage('');
  };
  ...
  const save = async () => {
    ...
    try {
      await apiClient.saveIntelligenceProvider({ provider: selected, model: form.model || undefined, base_url: selected === 'gemini' ? undefined : form.base_url || undefined, api_key: form.api_key || undefined, priority: form.priority });
      setForm((f) => ({ ...f, api_key: '' })); await load(); setMessage(`${current?.label || selected} configuration saved. Activation remains a separate operator action.`);
  ...
  const test = async () => {
    ...
    try {
      const result = await apiClient.testIntelligenceProvider({ provider: selected, model: form.model || undefined, base_url: selected === 'gemini' ? undefined : form.base_url || undefined, api_key: form.api_key || undefined });
      setForm((f) => ({ ...f, api_key: '' })); await load(); setMessage(`Connection test passed: ${result.provider} / ${result.model}`);
  ...
  const healthCheck = async (all = false) => {
    ...
    try {
      const result = await apiClient.runIntelligenceProviderHealthCheck(all ? {} : { provider: selected, model: form.model || undefined, base_url: selected === 'gemini' ? undefined : form.base_url || undefined, api_key: form.api_key || undefined });
    ...
    } catch (e: any) { setMessage(e.message || 'Provider health check failed'); } finally { setBusy(''); }
  };

  const activate = async () => {
    setBusy('activate'); setMessage('');
    try { const result = await apiClient.activateIntelligenceProvider(selected); await load(); setMessage(`${current?.label || selected} activated as the operator-preferred provider. Other active eligible providers remain available for dynamic routing/fallback.`); void result; }
    catch (e: any) { setMessage(e.message || 'Unable to activate provider'); } finally { setBusy(''); }
  };

  const deactivate = async () => {
    setBusy('deactivate'); setMessage('');
    try { await apiClient.deactivateIntelligenceProvider(selected); await load(); setMessage(`${current?.label || selected} manually deactivated. It is excluded from runtime routing and fallback regardless of health or priority.`); }
    catch (e: any) { setMessage(e.message || 'Unable to deactivate provider'); } finally { setBusy(''); }
  };

  const saveHealthPolicy = async () => {
    setBusy('health-policy'); setMessage('');
    try { const result = await apiClient.updateIntelligenceProviderHealthPolicy(healthPolicy); setHealthPolicy(result); await load(); setMessage(`Automatic health checks ${result.enabled ? 'enabled' : 'disabled'} at ${fmtInterval(result.interval_seconds)} intervals.`); }
    catch (e: any) { setMessage(e.message || 'Unable to update automatic health policy'); } finally { setBusy(''); }
  };

  const effectiveForm = current && current.configuration_valid === false ? (PROVIDER_CATALOG.find((p) => p.provider === selected) || current) : current;
  const nextHealthCheck = lastHealthCheck ? new Date(new Date(lastHealthCheck).getTime() + healthPolicy.interval_seconds * 1000) : null;
  const traceAttempts = latestTrace?.attempts || [];
