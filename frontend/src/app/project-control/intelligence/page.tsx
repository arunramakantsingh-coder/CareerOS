...
const PROVIDER_CATALOG: Provider[] = [
  {
    ...
    active: false,
    priority: 100,
    capabilities: ['local', 'private', 'structured_output'],
  },
  {
    provider: 'openrouter',
    label: 'OpenRouter',
    ...
    active: false,
    priority: 100,
    capabilities: ['routing', 'model_choice', 'structured_output'],
  },
  {
    provider: 'openai',
    label: 'OpenAI',
    ...
    active: false,
    priority: 100,
    capabilities: ['reasoning', 'structured_output', 'vision'],
  },
  {
    provider: 'gemini',
    label: 'Google Gemini',
  ...
  },
  {
    ...
    active: false,
    priority: 100,
    capabilities: ['reasoning', 'structured_output', 'document_intelligence'],
  },
  {
    provider: 'xai',
    label: 'xAI',
    ...
    active: false,
    priority: 100,
    capabilities: ['reasoning', 'vision', 'web_search'],
  },
  {
    provider: 'groq',
    label: 'Groq',
  ...
  },
  {
    ...
    active: false,
    priority: 100,
    capabilities: ['reasoning', 'coding', 'structured_output', 'long_context'],
  },
];
...
function catalogWithRuntime(runtime: Provider[] = []) {
  const byName = new Map(runtime.map((p) => [p.provider, p]));
  return PROVIDER_CATALOG.map((catalogProvider) => ({
    ...catalogProvider,
    ...(byName.get(catalogProvider.provider) || {}),
  }));
}
...
export default function IntelligenceEngine() {
  ...
  const load = async () => {
    ...
    const observabilityResult = results[2];
    const providerHealthResult = results[3];

    let runtimeProviders: Provider[] = [];

    if (providerResult.status === 'fulfilled') {
      runtimeProviders = providerResult.value.providers || [];
      setActiveProvider(providerResult.value.active_provider || '');
    }

    const healthData =
      providerHealthResult.status === 'fulfilled'
        ? providerHealthResult.value
        : null;

    const merged = catalogWithRuntime(runtimeProviders).map((p) => {
      const healthEntry = (healthData?.providers || []).find(
        (x: any) => x.provider === p.provider,
      );

      return {
        ...p,
        health: healthEntry?.health || p.health,
        health_fresh: Boolean(
          healthEntry?.health_fresh ?? p.health_fresh,
        ),
      };
    });
    ...
    if (observabilityResult.status === 'fulfilled') {
      setObs(observabilityResult.value);
    }
    ...
    const current =
      merged.find((p) => p.provider === selected) ||
      merged.find((p) => p.active) ||
      merged[0];
    ...
    if (current) {
      ...
      setDailyLimit(
        current.routing_policy?.daily_request_limit
          ? String(current.routing_policy.daily_request_limit)
          : '',
      );
    }
    ...
    if (!providersAvailable) {
      setMessage(
        'CareerOS API is currently unreachable. Showing the complete provider catalog; live configuration, health and routing data will appear when the backend reconnects.',
      );
    } else {
      setMessage((previous) =>
        previous.startsWith('CareerOS API is currently unreachable')
          ? ''
          : previous,
      );
    }
  ...
  useEffect(() => {
    load().catch((e: any) =>
      setMessage(e.message || 'Unable to load Intelligence Engine'),
    );
  }, []);
  ...
  const current = useMemo(
    () => providers.find((p) => p.provider === selected),
    [providers, selected],
  );

  const currentFresh = Boolean(current?.health_fresh);
  const currentHealthy =
    current?.health?.status === 'healthy' && currentFresh;

  const healthyProviders = providers.filter(
    (p) => p.health_fresh && p.health?.status === 'healthy',
  );
  ...
  const selectProvider = (name: string) => {
    ...
    setDailyLimit(
      p.routing_policy?.daily_request_limit
        ? String(p.routing_policy.daily_request_limit)
        : '',
    );
    setMessage('');
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      await load();
      setMessage(
        `${current?.label || selected} configuration saved. Run a health check before activation or AI use.`,
      );
    } catch (e: any) {
  ...
  const healthCheck = async (all = false) => {
    ...
    try {
      ...
      const ok = (result.providers || []).filter(
        (p: any) => p.health?.status === 'healthy',
      ).length;
  ...
  const activate = async () => {
    ...
    try {
      ...
      setMessage(
        `${current?.label || selected} is now the global active AI provider. Routing will use only providers with a fresh successful health check.`,
      );
    } catch (e: any) {
  ...
  const selectedProviderLabel =
    current?.label || activeLive?.label || selected || 'selected provider';
    ...
    <CareerOSShell>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <p className="mt-2 font-semibold">
            {healthyProviders.length} / {providers.filter((p) => p.configured).length}
          </p>
        </Card>

        <Card>
          <p className="text-xs text-muted-foreground">Health gate</p>
          <p className="mt-2 font-semibold">
            {healthyProviders.length ? 'Routing ready' : 'Health check required'}
          </p>
        </Card>

        <Card>
          <p className="text-xs text-muted-foreground">Health TTL</p>
          <p className="mt-2 font-semibold">{Math.round(healthTtl / 60)} min</p>
        </Card>
      </div>
      ...
      <Card className="mt-5" title="Provider registry">
        <div className="mb-4 rounded-xl border bg-muted/20 p-3 text-xs text-muted-foreground">
          The complete provider catalog is always visible. Configuration, health,
          activation and routing status are loaded from the CareerOS backend when
          available. Unconfigured providers remain selectable so the catalog does
          not disappear during a backend/API outage.
        </div>
            ...
            return (
              ...
              >
                ...
                <p className="mt-3 text-xs text-muted-foreground">
                  {p.model || 'Model not selected'}
                </p>

                {p.configured && p.api_key_present && (
                  <p className="mt-1 text-[11px] text-muted-foreground">
                    Key saved · ••••{p.api_key_last4}
                  </p>
                )}

                <div className="mt-3 flex flex-wrap gap-1">
                  {(p.capabilities || []).slice(0, 5).map((x) => (
                    <Badge key={x}>{x}</Badge>
                  ))}
                </div>

                <p className="mt-3 text-[11px] text-muted-foreground">
                  Checked: {fmtTime(p.health?.checked_at)}
                </p>
              </button>
            );
          })}
      ...
      {current && (
        <Card className="mt-5" title={`${current.label} configuration`}>
          <div className="rounded-xl border bg-muted/20 p-3 text-xs leading-5 text-muted-foreground">
            Save configuration first, then run a non-generative health check.
            Activation and AI routing require a recent successful health check.
            Health checks are operational controls and are tracked separately from
            generation usage.
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Button
              onClick={activate}
              disabled={!!busy || current.active || !currentHealthy}
            >
              {busy === 'activate'
                ? 'Activating…'
                : current.active
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            A successful health check is tied to the configured provider/model and
            expires after {Math.round(healthTtl / 60)} minutes. Changing provider
            configuration requires another health check.
          </p>
        </Card>
      )}
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <Card title="Task-based routing">
          <p className="text-sm text-muted-foreground">
            Routing is health-gated: configured → healthy/fresh → capability match
            → priority. The active provider is preferred only when it passes the
            health gate.
          </p>
          ...
          <div className="mt-4 rounded-xl border p-3 text-xs text-muted-foreground">
            Healthy routing order:{' '}
            {(obs?.routing?.healthy_provider_order || []).join(' → ') ||
              'No healthy provider yet'}
          </div>
        </Card>

        <Card title="Fallback routing">
          <p className="text-sm text-muted-foreground">
            Fallback is allowed only among configured, compatible, recently healthy
            providers. A failed generation is recorded and the next healthy
            candidate may be attempted according to policy.
          </p>
        ...
        <Card title="Usage & cost controls">
          <p className="text-sm text-muted-foreground">
            Usage is recorded at the generation boundary. Health checks are tracked
            separately so operational checks do not inflate AI request usage. Daily
            request limits are enforced before generation.
          </p>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-xl border p-3">
              ...
              <p className="mt-1 text-lg font-semibold">
                {liveProviders.some(
                  (p) => p.telemetry?.estimated_cost !== undefined,
                )
                  ? `$${liveProviders
                      .reduce(
                        (n, p) =>
                          n + Number(p.telemetry?.estimated_cost || 0),
                        0,
                      )
                      .toFixed(4)}`
                  : 'Not reported'}
          ...
          <div className="mt-4 flex flex-wrap items-end gap-3">
            <div className="min-w-48">
              <label className="text-xs font-medium">
                Daily request limit for {selectedProviderLabel}
              </label>
              <input
                type="number"
                min={1}
        ...
        <Card title="Health / latency / quota">
          <p className="text-sm text-muted-foreground">
            Operational health is measured separately from generation telemetry.
            Provider-reported quota/rate-limit signals are shown only when returned
            by the provider; CareerOS limits remain separate.
          </p>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody>
                {providers.map((p) => {
                  ...
                  const fresh = Boolean(p.health_fresh);
                  const quota =
                    t.quota && Object.keys(t.quota).length
                      ? JSON.stringify(t.quota)
                      : 'Not reported';
                    ...
                    >
                      ...
                      <td className="px-2 py-2">
                        ...
                        <div className="mt-1 text-[10px] text-muted-foreground">
                          {fmtTime(t.checked_at)}
                        </div>
                      </td>

                      <td className="px-2 py-2">
                        {fmtMs(t.avg_latency_ms)}
                      </td>

                      <td className="px-2 py-2">
                        {fmtMs(t.p95_latency_ms)}
                      </td>

                      <td className="px-2 py-2">
                        {t.failed_checks || 0}
                        {t.consecutive_failures
                          ? ` (${t.consecutive_failures} consecutive)`
                          : ''}
                      </td>
      ...
      <Card className="mt-5" title="Architecture boundary">
        <div className="space-y-3 text-sm leading-6 text-muted-foreground">
          <p>
            <strong className="text-foreground">One gateway:</strong> CareerOS
            modules do not call individual AI vendors directly. Provider health is a
            control-plane operation through the gateway.
          </p>

          <p>
            <strong className="text-foreground">Health before routing:</strong>{' '}
            configuration is only eligibility metadata. A provider must pass a recent
            non-generative health check before activation or AI routing. Routing then
            applies task capability, priority, policy and fallback.
          </p>

          <p>
            <strong className="text-foreground">AI is not the system of record:</strong>{' '}
            AI produces candidate facts, classifications and recommendations.
            CareerOS application services remain authoritative for persistence,
            validation, provenance, permissions and state transitions.
          </p>
        </div>
      </Card>
    </CareerOSShell>
