...
const groups: NavGroup[] = [
  ...
  { id: 'identity', title: 'Professional Identity', items: [
    ['Profile', '/profile', '◎'], ['CV & Documents', '/documents', '▤'], ['Profile Setup', '/profile/setup', '◌'],
    ['Evidence Library', '/evidence-library', '▥'], ['Career Vault', '/career-vault', '◇'], ['Personas', '/personas', '◍']
  ] },
...
export function CareerOSShell({ children }: { children: ReactNode }) {
  ...
  const [user, setUser] = useState<any>(null); const [loading, setLoading] = useState(true); const [menuOpen, setMenuOpen] = useState(false); const [mobileOpen, setMobileOpen] = useState(false);
  const { theme, setTheme } = useTheme();
  const groupId = useMemo(() => activeGroup(pathname), [pathname]); const group = groups.find(x => x.id === groupId) || groups[0];
  useEffect(() => { let live=true; (async()=>{ if(pathname==='/login'||pathname==='/register'){setLoading(false);return;} if(!apiClient.hasToken()){router.replace('/login');return;} try{const me=await apiClient.me();if(live)setUser(me);}catch{apiClient.clearToken();router.replace('/login')}finally{if(live)setLoading(false)}})(); return()=>{live=false}; },[pathname,router]);
  ...
  const logout=()=>{apiClient.clearToken();router.replace('/login')}; const isActive=(href:string)=>href==='/'?pathname==='/':pathname===href||pathname.startsWith(`${href}/`);
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[236px] border-r bg-sidebar md:flex md:flex-col">
      ...
      <nav className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-3" aria-label="CareerOS domains">{groups.map(g=><button key={g.id} type="button" onClick={()=>selectGroup(g)} className={`mb-2 flex w-full items-center justify-between rounded-xl px-3 py-3 text-left transition ${g.id===groupId?'bg-primary/10 text-primary ring-1 ring-primary/20':'text-muted-foreground hover:bg-muted/70 hover:text-foreground'}`}><span><span className="block text-[10px] font-bold uppercase tracking-[.17em]">{g.title}</span><span className="mt-1 block text-[11px] opacity-70">{g.items.length} workspace {g.items.length===1?'area':'areas'}</span></span><span className="text-xs">{g.id===groupId?'●':'○'}</span></button>)}</nav>
    ...
    <div className="md:pl-[236px]">
      <header className="sticky top-0 z-30 border-b bg-background/90 backdrop-blur-xl"><div className="flex min-h-[68px] items-center gap-3 px-4 py-2 sm:px-6"><button className="grid h-9 w-9 place-items-center rounded-lg border bg-card/60 md:hidden" onClick={()=>setMobileOpen(v=>!v)} aria-label="Open application navigation">☰</button><div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-primary/10 text-xs font-bold text-primary ring-1 ring-primary/20">{(user?.name||user?.email||'A').slice(0,1).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{user?.name||'CareerOS'}</p><p className="hidden truncate text-[11px] text-muted-foreground sm:block">Professional identity · evidence-first career operating system</p></div><div className="hidden items-center rounded-xl border bg-card/60 p-1 lg:flex">{themes.map(t=><button key={t.key} type="button" onClick={()=>setTheme(t.key)} className={`rounded-lg px-3 py-1.5 text-[11px] font-semibold transition ${theme===t.key?'bg-primary text-primary-foreground shadow-sm':'text-muted-foreground hover:text-foreground'}`}>{t.label}</button>)}</div><Link href="/settings" className="rounded-lg border bg-card/60 px-3 py-2 text-xs font-semibold transition hover:bg-muted">Settings</Link><div className="relative"><button type="button" aria-label="More application options" onClick={()=>setMenuOpen(v=>!v)} className="grid h-9 w-9 place-items-center rounded-lg border bg-card/60 text-lg hover:bg-muted">⋮</button>{menuOpen&&<div className="absolute right-0 top-11 w-60 rounded-xl border bg-card p-2 shadow-xl"><Link onClick={()=>setMenuOpen(false)} href="/project-tracker" className="block rounded-lg px-3 py-2 text-sm hover:bg-muted">Project tracker</Link><Link onClick={()=>setMenuOpen(false)} href="/bug-tracker" className="block rounded-lg px-3 py-2 text-sm hover:bg-muted">Bug tracker</Link><Link onClick={()=>setMenuOpen(false)} href="/roadmap" className="block rounded-lg px-3 py-2 text-sm hover:bg-muted">Roadmap</Link><Link onClick={()=>setMenuOpen(false)} href="/settings" className="block rounded-lg px-3 py-2 text-sm hover:bg-muted">Application settings</Link><button onClick={logout} className="mt-1 w-full rounded-lg px-3 py-2 text-left text-sm text-red-500 hover:bg-muted">Sign out</button></div>}</div></div>
      <div className="border-t bg-card/25 px-3 py-2 sm:px-4"><div className="flex items-center gap-2"><div className="hidden shrink-0 px-2 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground lg:block">{group.title}</div><nav className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto pb-0.5" aria-label={`${group.title} workspace`}>{group.items.map(([label,href,icon])=><Link key={href} href={href} title={label} className={`flex min-w-max items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition ${isActive(href)?'bg-primary/10 text-primary shadow-[inset_0_-2px_0_hsl(var(--primary))]':'text-muted-foreground hover:bg-muted/60 hover:text-foreground'}`}><span className="text-[11px] opacity-90" aria-hidden="true">{icon}</span><span>{label}</span></Link>)}</nav></div></div></header>
