...
const groups: NavGroup[] = [
  ...
  { id: 'connections', title: 'Connections', items: [
    ['Organizations', '/connections/organizations', '▦'], ['Recruiters', '/connections/recruiters', '◎'], ['People', '/connections/people', '◌']
  ] },
  ...
  { id: 'interview', title: 'Interview & Insight', items: [
    ['Interview Preparation', '/interviews', '◉'], ['Live Interview', '/live-interview', '◍'], ['Interview Intelligence', '/analytics', '⌁'], ['Insights', '/insights', '◇']
  ] },
...
const searchItems = groups.flatMap(group => group.items.map(([label, href]) => ({ label, href, group: group.title })));

function activeGroup(pathname: string, visibleGroups: NavGroup[]) {
  return visibleGroups.find(group => group.items.some(([, href]) => href === '/' ? pathname === '/' : pathname === href || pathname.startsWith(`${href}/`)))?.id || 'overview';
}
...
export function CareerOSShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        apiClient.clearToken(); router.replace('/login');
  ...
  const searchResults = useMemo(() => {
    ...
    if (!q) return [];
    return searchItems.filter(item => item.label.toLowerCase().includes(q) || item.group.toLowerCase().includes(q)).slice(0, 8);
  }, [search]);
      ...
      <header className="sticky top-0 z-30 border-b bg-background/90 backdrop-blur-xl">
        <div className="flex min-h-[68px] items-center gap-3 px-4 py-2 sm:px-6">
          ...
          <div className="relative hidden w-full max-w-[360px] sm:block">
            <div className="flex items-center rounded-xl border bg-card/70 px-3 py-2 shadow-sm"><span className="mr-2 text-sm text-muted-foreground">⌕</span><input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') setSearch(''); if (e.key === 'Enter' && searchResults[0]) { setSearch(''); router.push(searchResults[0].href); } }} placeholder="Search CareerOS" aria-label="Search CareerOS" className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground"/><kbd className="hidden rounded-md border px-1.5 py-0.5 text-[10px] text-muted-foreground lg:block">/</kbd></div>
            {searchResults.length > 0 && <div className="absolute right-0 top-12 z-50 w-full overflow-hidden rounded-xl border bg-card shadow-2xl">{searchResults.map(item => <button key={`${item.group}-${item.href}`} type="button" onClick={() => { setSearch(''); router.push(item.href); }} className="flex w-full items-center justify-between px-3 py-2.5 text-left text-sm hover:bg-muted"><span>{item.label}</span><span className="text-[10px] text-muted-foreground">{item.group}</span></button>)}</div>}
          </div>

          <div className="relative shrink-0">
            <button type="button" aria-label="User account menu" onClick={() => setMenuOpen(v => !v)} className="grid h-10 w-10 place-items-center rounded-xl border bg-card/60 text-lg font-semibold transition hover:border-primary/40 hover:bg-muted">⋮</button>
            {menuOpen && <div className="absolute right-0 top-12 z-50 w-72 rounded-2xl border bg-card p-2 shadow-2xl">
              <div className="border-b px-3 py-2"><p className="text-xs font-semibold">{user?.name || 'CareerOS account'}</p><p className="mt-0.5 truncate text-[11px] text-muted-foreground">{user?.email || ''}</p></div>
              <Link onClick={() => setMenuOpen(false)} href="/profile" className="mt-1 block rounded-lg px-3 py-2 text-sm hover:bg-muted">My Profile</Link>
              <Link onClick={() => setMenuOpen(false)} href="/settings?section=account" className="block rounded-lg px-3 py-2 text-sm hover:bg-muted">Account & Security</Link>
              <div className="mt-1 rounded-lg border bg-background/40 p-2"><p className="px-2 py-1 text-[10px] font-bold uppercase tracking-[.16em] text-muted-foreground">Appearance</p><div className="mt-1 grid grid-cols-3 gap-1">{themes.map(t => <button key={t.key} type="button" onClick={() => setTheme(t.key)} className={`rounded-lg px-2 py-2 text-[11px] font-semibold transition ${theme === t.key ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}>{t.label}</button>)}</div></div>
              <Link onClick={() => setMenuOpen(false)} href="/settings" className="mt-1 block rounded-lg px-3 py-2 text-sm hover:bg-muted">Application Settings</Link>
              <button onClick={logout} className="mt-1 w-full rounded-lg px-3 py-2 text-left text-sm text-red-500 hover:bg-muted">Sign out</button>
            </div>}
          </div>
        </div>
