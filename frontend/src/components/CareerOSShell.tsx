...
const groups: NavGroup[] = [
  ...
  { title:'Intelligence', items:[['Interviews','/interviews','◉'],['Live Interview','/live-interview','◍'],['Analytics','/analytics','⌁']] },
  { title:'Global Mobility', items:[['Global Mobility','/global-mobility','◎']] },
] as const;

const topNav: NavItem[] = [
  ['Home','/','⌂'], ['Profile','/profile','◎'], ['Career Vault','/career-vault','◇'],
  ['Documents','/documents','▤'], ['Personas','/personas','◌'], ['Jobs','/jobs','▣'],
  ['Applications','/applications','☷'], ['Application Studio','/application-studio','✦'],
...
export function CareerOSShell({children}:{children:ReactNode}){
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[286px] border-r bg-sidebar md:flex md:flex-col">
      <div className="border-b px-4 py-4">
        <Link href="/" className="flex items-center gap-3">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary text-primary-foreground text-lg shadow-[0_0_18px_hsl(var(--primary)/.18)]">◇</span>
      ...
      <nav className="careeros-nav-scroll min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-3">
        {groups.map(g=><div key={g.title} className="mb-5">
          ...
          <div className="space-y-1">
            {g.items.map(([label,href,icon])=>{
              ...
              return <Link key={href} href={href} className={`group flex h-10 items-center gap-3 rounded-lg px-3 text-sm transition ${active?'bg-primary/10 font-medium text-primary shadow-[inset_2px_0_0_hsl(var(--primary))]':'text-muted-foreground hover:bg-muted/70 hover:text-foreground'}`}>
                <span className="w-4 text-center text-sm opacity-90">{icon}</span>{label}
              </Link>
            })}
          </div>
        </div>)}
      ...
      <div className="border-t p-3">
        <div className="rounded-xl border bg-card/70 p-2.5">
          <div className="flex items-center gap-3">
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-primary/10 text-xs font-bold text-primary ring-1 ring-primary/15">{(user?.name||user?.email||'A').slice(0,1).toUpperCase()}</span>
            <span className="min-w-0"><span className="block truncate text-xs font-semibold">{user?.name||'Personal Workspace'}</span><span className="block truncate text-[11px] text-muted-foreground">{user?.email||'CareerOS'}</span></span>
          </div>
    ...
    <div className="md:pl-[286px]">
      <header className="sticky top-0 z-30 border-b bg-background/90 backdrop-blur-xl">
        <div className="flex min-h-[68px] items-center gap-3 px-4 py-2 sm:px-6">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-bold">{user?.name||'CareerOS'}</p>
            <p className="hidden truncate text-[11px] text-muted-foreground sm:block">Your professional identity · evidence-first career workspace</p>
          </div>
          <div className="hidden items-center rounded-xl border bg-card/60 p-1 sm:flex">
            {themes.map(t=><button key={t.key} type="button" onClick={()=>setTheme(t.key)} className={`rounded-lg px-3 py-1.5 text-[11px] font-semibold transition ${theme===t.key?'bg-primary text-primary-foreground shadow-sm':'text-muted-foreground hover:text-foreground'}`}>{t.label}</button>)}
          </div>
        ...
        <div className="border-t bg-card/20 px-3 py-2">
          <nav className="careeros-nav-scroll flex gap-1 overflow-x-auto pb-0.5" aria-label="Primary navigation">
            {topNav.map(([label,href,icon])=>{
              const active=isActive(href);
              return <Link key={href} href={href} className={`flex min-w-max items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition ${active?'bg-primary/10 text-primary shadow-[inset_0_-2px_0_hsl(var(--primary))]':'text-muted-foreground hover:bg-muted/60 hover:text-foreground'}`}>
      ...
      <div className="border-b bg-background px-3 py-2 md:hidden">
        <select value={pathname} onChange={e=>router.push(e.target.value)} className="w-full rounded-lg border bg-card px-3 py-2.5 text-sm">
          {[...groups.flatMap(g=>g.items),['Settings','/settings','⚙'] as NavItem].map(([label,href])=><option key={href} value={href}>{label}</option>)}
        </select>
      </div>

      <main className="min-h-[calc(100vh-7rem)] px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto w-full max-w-[1400px]">{children}</div></main>
    </div>
  </div>
}
