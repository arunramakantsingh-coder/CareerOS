...
const groups: NavGroup[] = [
  ...
  { id: 'opportunity', title: 'Opportunity', items: [
    ...
    ['Resume Studio', '/resume-studio', '▤'], ['Company Intelligence', '/company-intelligence', '⌁']
  ] },
  { id: 'interview', title: 'Interview & Insight', items: [
    ['Interviews', '/interviews', '◉'], ['Live Interview', '/live-interview', '◍'], ['Analytics', '/analytics', '⌁']
  ] },
  { id: 'global', title: 'Global', items: [['Global Mobility', '/global-mobility', '◎']] },
  { id: 'control', title: 'Project Control', items: [
    ['Project Tracker', '/project-tracker', '▥'], ['Bug Tracker', '/bug-tracker', '⚠'], ['Roadmap', '/roadmap', '➜'], ['Application Settings', '/settings', '⚙']
  ] }
];
...
function activeGroup(pathname: string) {
  return groups.find(group => group.items.some(([, href]) => href === '/' ? pathname === '/' : pathname === href || pathname.startsWith(`${href}/`)))?.id || 'overview';
}
...
export function CareerOSShell({ children }: { children: ReactNode }) {
  ...
  const groupId = useMemo(() => activeGroup(pathname), [pathname]);
  ...
  if (pathname !== '/login' && pathname !== '/register' && loading) {
    return <div className="min-h-screen grid place-items-center bg-background"><div className="techno-surface rounded-2xl border bg-card px-6 py-5 shadow-sm">Loading CareerOS…</div></div>;
  }

  const logout = () => { apiClient.clearToken(); router.replace('/login'); };
  const isActive = (href: string) => href === '/' ? pathname === '/' : pathname === href || pathname.startsWith(`${href}/`);
  const selectGroup = (next: NavGroup) => {
    setMobileOpen(false);
    const [, href] = next.items[0];
    router.push(href);
  };
    ...
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[236px] border-r bg-sidebar md:flex md:flex-col">
      <div className="border-b px-4 py-4">
        <Link href="/" className="flex items-center gap-3">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary text-lg text-primary-foreground shadow-[0_0_22px_hsl(var(--primary)/.22)]">◇</span>
      ...
      <nav className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-3" aria-label="CareerOS domains">
        {groups.map(g => <button key={g.id} type="button" onClick={() => selectGroup(g)} className={`mb-2 flex w-full items-center justify-between rounded-xl px-3 py-3 text-left transition ${g.id === groupId ? 'bg-primary/10 text-primary ring-1 ring-primary/20' : 'text-muted-foreground hover:bg-muted/70 hover:text-foreground'}`}>
          <span><span className="block text-[10px] font-bold uppercase tracking-[.17em]">{g.title}</span><span className="mt-1 block text-[11px] opacity-70">{g.items.length} workspace {g.items.length === 1 ? 'area' : 'areas'}</span></span>
          <span className="text-xs">{g.id === groupId ? '●' : '○'}</span>
        </button>)}
      </nav>
    ...
    <div className="md:pl-[236px]">
      <header className="sticky top-0 z-30 border-b bg-background/90 backdrop-blur-xl">
        <div className="flex min-h-[68px] items-center gap-3 px-4 py-2 sm:px-6">
          ...
          <div className="relative"><button type="button" aria-label="More application options" onClick={() => setMenuOpen(v => !v)} className="grid h-9 w-9 place-items-center rounded-lg border bg-card/60 text-lg hover:bg-muted">⋮</button>{menuOpen && <div className="absolute right-0 top-11 w-60 rounded-xl border bg-card p-2 shadow-xl">
            <Link onClick={() => setMenuOpen(false)} href="/project-tracker" className="block rounded-lg px-3 py-2 text-sm hover:bg-muted">Project tracker</Link><Link onClick={() => setMenuOpen(false)} href="/bug-tracker" className="block rounded-lg px-3 py-2 text-sm hover:bg-muted">Bug tracker</Link><Link onClick={() => setMenuOpen(false)} href="/roadmap" className="block rounded-lg px-3 py-2 text-sm hover:bg-muted">Roadmap</Link><Link onClick={() => setMenuOpen(false)} href="/settings" className="block rounded-lg px-3 py-2 text-sm hover:bg-muted">Application settings</Link><button onClick={logout} className="mt-1 w-full rounded-lg px-3 py-2 text-left text-sm text-red-500 hover:bg-muted">Sign out</button>
          </div>}</div>
        </div>
        <div className="border-t bg-card/25 px-3 py-2 sm:px-4"><div className="flex items-center gap-2">
          <div className="hidden shrink-0 px-2 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground lg:block">{group.title}</div>
          <nav className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto pb-0.5" aria-label={`${group.title} workspace`}>
            {group.items.map(([label, href, icon]) => <Link key={href} href={href} title={label} className={`flex min-w-max items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition ${isActive(href) ? 'bg-primary/10 text-primary shadow-[inset_0_-2px_0_hsl(var(--primary))]' : 'text-muted-foreground hover:bg-muted/60 hover:text-foreground'}`}><span className="text-[11px] opacity-90">{icon}</span><span>{label}</span></Link>)}
          </nav>
        </div></div>
      </header>

      {mobileOpen && <div className="fixed inset-0 z-50 bg-background/95 p-4 backdrop-blur-xl md:hidden"><div className="mb-4 flex items-center justify-between"><p className="font-semibold">CareerOS navigation</p><button onClick={() => setMobileOpen(false)} className="grid h-9 w-9 place-items-center rounded-lg border">×</button></div>
        <div className="mb-4 grid gap-2 sm:grid-cols-2">{groups.map(g => <button key={g.id} onClick={() => selectGroup(g)} className={`rounded-xl border px-3 py-3 text-left ${g.id === groupId ? 'border-primary bg-primary/10 text-primary' : 'bg-card text-muted-foreground'}`}><span className="block text-[10px] font-bold uppercase tracking-[.17em]">{g.title}</span></button>)}</div>
        <div className="max-h-[calc(100vh-13rem)] overflow-auto rounded-xl border bg-card p-2">{group.items.map(([label, href, icon]) => <Link key={href} href={href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 rounded-lg px-3 py-3 text-sm ${isActive(href) ? 'bg-primary/10 font-semibold text-primary' : 'text-muted-foreground hover:bg-muted'}`}><span>{icon}</span>{label}</Link>)}</div>
      </div>}
...
export function Card({ title, children, className = '' }: { title?: string; children: ReactNode; className?: string }) {
  return <section className={`techno-surface rounded-xl border bg-card shadow-sm ${className}`}>{title && <div className="border-b px-5 py-4"><h2 className="text-sm font-semibold">{title}</h2></div>}<div className="p-5">{children}</div></section>;
}

export function Badge({ children, tone = 'muted' }: { children: ReactNode; tone?: 'muted' | 'good' | 'warn' | 'blue' }) {
  const c = { muted: 'bg-muted text-muted-foreground', good: 'bg-emerald-500/10 text-emerald-500', warn: 'bg-amber-500/10 text-amber-500', blue: 'bg-primary/10 text-primary' }[tone];
  return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${c}`}>{children}</span>;
}

export function Button({ children, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button {...props} className={`rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground shadow-sm transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 ${props.className || ''}`}>{children}</button>;
}
