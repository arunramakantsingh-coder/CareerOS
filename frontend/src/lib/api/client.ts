...
function apiErrorMessage(data: any, fallback: string): string {
  ...
  if (Array.isArray(detail)) { const messages = detail.map(item => typeof item === 'string' ? item : item?.msg || item?.message || 'Validation error').filter(Boolean); if (messages.length) return messages.join(' • '); }
  if (detail && typeof detail === 'object') { if (typeof detail.msg === 'string') return detail.msg; if (typeof detail.message === 'string') return detail.message; try { return JSON.stringify(detail); } catch { return fallback; } }
  return fallback;
}
class ApiClient {
  ...
  clearToken(): void { if (typeof window === 'undefined') return; localStorage.removeItem('access_token'); localStorage.removeItem('careeros_token'); localStorage.removeItem('user'); }
  private async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    const headers = new Headers(options.headers); if (!headers.has('Content-Type') && options.body) headers.set('Content-Type', 'application/json'); const token = this.token(); if (token) headers.set('Authorization', `Bearer ${token}`);
    let response: Response; try { response = await fetch(`${resolveApiBaseUrl()}${endpoint}`, { ...options, headers, cache: 'no-store' }); } catch { throw new Error(`Unable to reach CareerOS API at ${resolveApiBaseUrl()}. Check that the backend is running and reachable from this device.`); }
  ...
  identityOverviewEnriched() { return this.get<any>('/api/v1/identity/overview-enriched'); }
