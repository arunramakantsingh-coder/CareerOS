...
class ApiClient {
  ...
  async uploadDocuments(files: File[], options: { category?: string; subcategory?: string; onProgress?: (index: number, progress: number) => void } = {}) {
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      if (options.category) formData.append('document_category', options.category);
      if (options.subcategory) formData.append('document_subcategory', options.subcategory);
      results.push(await this.request<any>('/api/v1/documents/upload', { method: 'POST', body: formData })); options.onProgress?.(index, 100);
    }
    return results;
  }
  uploadBulk(files: File[], options: { category?: string; subcategory?: string } = {}) {
    ...
    return this.request<any>('/api/v1/documents/bulk-upload', { method: 'POST', body: formData });
  }
}
