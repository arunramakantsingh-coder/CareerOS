...
class AnthropicProvider:
    ...
    async def generate(self, *, prompt: str, system: str | None, response_schema: dict[str, Any] | None, temperature: float) -> ProviderResult:
        ...
        payload: dict[str, Any] = {"model": self.model, "max_tokens": 8192, "messages": messages, "temperature": temperature}
        if system:
            payload["system"] = system
        if response_schema:
...
def build_provider(config: dict[str, str], timeout: float) -> AIProvider:
    ...
    if provider_name == "anthropic":
        return AnthropicProvider(api_key, model or "claude-sonnet-4-5", base_url or "https://api.anthropic.com", timeout)
