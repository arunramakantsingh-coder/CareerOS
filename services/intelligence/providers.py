...
class OllamaProvider:
    name = "ollama"
    def __init__(self, base_url: str, model: str, timeout: float) -> None: self.base_url, self.model, self.timeout = base_url.rstrip("/"), model, timeout
    async def generate(self, *, prompt: str, system: str | None, response_schema: dict[str, Any] | None, temperature: float) -> ProviderResult:
...
class OpenAICompatibleProvider:
    def __init__(self, name: str, api_key: str, model: str, base_url: str, timeout: float) -> None: self.name, self.api_key, self.model, self.base_url, self.timeout = name, api_key, model, base_url.rstrip("/"), timeout
    async def generate(self, *, prompt: str, system: str | None, response_schema: dict[str, Any] | None, temperature: float) -> ProviderResult:
    ...
    async def health(self) -> dict[str, Any]: return {"provider": self.name, "configured": bool(self.api_key and self.model), "model": self.model or None, "reachable": None, "note": "Reachability is checked on generation to avoid quota use."}
...
def build_provider(config: dict[str, str], timeout: float) -> AIProvider:
    ...
    if provider_name == "gemini": return GeminiProvider(api_key or config.get("GEMINI_API_KEY", ""), model or "gemini-3.5-flash-lite", timeout)
